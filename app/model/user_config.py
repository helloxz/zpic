# 用户配置表
from sqlalchemy import Integer, String, DateTime,BigInteger,true,text
...
class UserConfigModel(Base):
    ...
    compress:Mapped[bool] = mapped_column(nullable=False,default=True,server_default=true(),comment="是否启用图片压缩")
    # 用户不可设置，只能由管理员开启
    ...
    # 用户总上传大小统计
    storage_bytes:Mapped[int] = mapped_column(BigInteger, default=0,server_default=text("0"), nullable=False, comment="用户存储总字节数")
    nsfw_count: Mapped[int] = mapped_column(Integer, default=0,server_default=text("0"), nullable=False, comment="色情图片数量")
    qrcode_count: Mapped[int] = mapped_column(Integer, default=0,server_default=text("0"), nullable=False, comment="二维码图片数量")
